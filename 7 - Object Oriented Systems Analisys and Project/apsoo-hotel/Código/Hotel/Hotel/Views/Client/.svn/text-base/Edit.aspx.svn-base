...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <style>
     ...
     <%if((String)ViewData["typeView"] == "ForRegistration"){ %>     
    ...
    <p>
     ...
     <% } else {%>     
    <h2><%=Model.IsAdd ? "New Client" : "Edit Client" %></h2>
    <% } %>
	...
	<% } %>
	 ...
	 <% String view = (String)ViewData["typeView"];	 
	 if(view != "ForRegistration") {
	    if(Model.IsAdd)
	        view = "AddSave";
	    else
	        view = "EditSave";
	 ...
	 <% using (Html.BeginForm(view, "Client", new { clientID = Model.Client.ID })) { %>	
        <fieldset>
