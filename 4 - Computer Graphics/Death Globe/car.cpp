...
static float rot = 90;    //Rotacao do carro
static float rod = 0;     //Rotacao das rodas
static float a= 0;        //Angulo de translacao
static float v = 0.1;       //Controla a velocidade do carro
static float t = 1;       //Controla o raio do globo
static float r = 700/t;   //Raio de translacao
static float d = 0.15*t;  //Distancia entre as rodas
...
void myinit(void)
{
    ...
    glNewList(TIRE,GL_COMPILE); //Display list da roda
    ...
    glNewList(CAR,GL_COMPILE); //Display List do carro
	glPushAttrib(GL_ALL_ATTRIB_BITS);
...
void display(void)
{
    ...
    rod += 10*v;//Controla a rotacao das rodas
    rot += 3.75*v;
    a += (3.141528/48)*v;    
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 'q': //sair do programa
		exit(0);
	break;
	case '+': //aumentar velocidade do carro
		v += 0.05;
	break;
	case '-': //diminuir velocidade do carro
		if (v>0.05) v -= 0.05;
	break;	
    }
    glutPostRedisplay();
