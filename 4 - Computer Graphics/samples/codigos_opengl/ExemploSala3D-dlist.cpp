...
// seta esquerda/direita - gira o observador
// seta superior/inferior - olha para cima ou para baixo
// PageUp/PageDown - eleva/desce o observador
...
TEX *parede, *chao, *teto;

// Objetos
OBJ *plano, *mesa, *mesapeq, *cadeira,
	*quadro, *porta, *janela, *lamp, *vidro, *ceu;
...
// Define modo inicial de aplicação da textura
...
void DesenhaParedes(void)
{
	...
	**********************************/
	...
	// 
	// Ajusta escala da textura
	SetaEscalaTextura(0.5,1.5); // 0.5 m x 1.5 m
...
void EspecificaParametrosVisualizacao(void)
{
	...
	gluPerspective(ang_cam,fAspect,0.1,2000);
...
void Teclado(unsigned char key, int x, int y)
{
	...
	switch(key)
	{
		...
		case 'Z':	obsX = obsX - 0.2*sina;
					obsZ = obsZ + 0.2*cosa;
					break;
		// Seta filtro de textura
		case 'f':	if(++filtro > 5) filtro = 0;
					SetaFiltroTextura(-1,filtros[filtro],filtros[filtro]);
					break;
		// Troca entre os modos de aplicação GL_REPLACE e GL_MODULATE
		case 'm':	if(modo == GL_REPLACE) modo = GL_MODULATE; else modo = GL_REPLACE;
...
{   
	...
	// Carrega as texturas
	parede = CarregaTextura("texturas/revmur036.jpg",true);
	chao   = CarregaTextura("texturas/015eyong.jpg",true);
	teto   = CarregaTextura("texturas/revmur037.jpg",true);
	...
	plano = CarregaObjeto("parede.obj",true);
	// Não queremos gerar display list para o plano,
	// pois a textura varia de acordo com o objeto
	// desenhado (parede, chão ou teto)
	DesabilitaDisplayList(plano);
	mesa = CarregaObjeto("mesagrande.obj",true);
	mesapeq = CarregaObjeto("mesapeq.obj",true);
	cadeira = CarregaObjeto("cadeira2.obj",true);
	quadro = CarregaObjeto("quadronegro.obj",true);
	porta = CarregaObjeto("porta.obj",true);
	janela = CarregaObjeto("janela.obj",true);
	lamp = CarregaObjeto("lampada.obj",true);
	// Como o estado da lâmpada varia, não
	// é interessante gerar display list para ela
	DesabilitaDisplayList(lamp);
	vidro = CarregaObjeto("vidro.obj",true);
	ceu  = CarregaObjeto("ceu2.obj",true);

	// Carrega objetos que podem estar sobre as mesas
	tipos[0] = CarregaObjeto("lapis.obj",true);
	tipos[1] = CarregaObjeto("livro.obj",true);
	tipos[2] = CarregaObjeto("papel1.obj",true);
	tipos[3] = CarregaObjeto("papel2.obj",true);
	tipos[4] = CarregaObjeto("papel3.obj",true);
	tipos[5] = CarregaObjeto("cuiabomba.obj",true);
	tipos[6] = CarregaObjeto("borracha.obj",true);
	...
	SetaFiltroTextura(-1,GL_LINEAR_MIPMAP_LINEAR,GL_LINEAR_MIPMAP_LINEAR);
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("ExemploSala3D com otimização via display lists");
