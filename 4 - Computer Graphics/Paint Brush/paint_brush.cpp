...
void myinit(void) //inicialização
{
...
void myReshape(GLsizei w, GLsizei h) //definir viewport
{        
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity(); 
...
void borralapis(int x, int y)  //função para desenhar quadrados
{
    y=wh-y;
    glBegin(GL_POLYGON); //formando quadrados
...
void clear(void) //limpar área de desenho
{
    glClearColor (rb, gb, bb, 1.0);
    glClear(GL_COLOR_BUFFER_BIT);       
}

int ferramenta(int x, int y)  //retorna a opção escolhida no menu do paint
{
    y = wh - y;
...
{   
    opcao = ferramenta(x,y); //determinar qual opção esolhida
    if (opcao == 0)
    {
       glLogicOp(GL_XOR); //cor da linha antes do último clique
    ...
    switch(draw_mode[0])
    {
      case(LINE): 
	      if(first == 1) 
	      {
            glBegin(GL_LINES); //desenha linha
			  glVertex2f(xm, ym);
			  glVertex2f(xmm, ymm);
		    glEnd();
		    glutSwapBuffers();
      ...
      case(RECTANGLE):
            ...
            if(first == 1)
	      {	
		    glRectf(xm, ym, xmm, ymm); //desenha retângulo
		    glutSwapBuffers();
	      }
	        xmm = x;
	        ymm = (wh - y);
            ...
            glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);  
           break;
       case(TRIANGLE): //desenhar triângulo
              if(count==2)
              {        
	          if(first == 1)
	          {
       ...
       case(BORRACHA):
              glLogicOp(GL_COPY);
	          glColor3f( rb, gb, bb); //definir cor de fundo para apagar
	          borralapis(x,y); //apagando com quadrados brancos
           break;
       case(LAPIS):
              glLogicOp(GL_COPY);
              glColor3f(r,g,b); //cor corrente
              borralapis(x,y);  //usar lápis com cor corrente             
	       break;
            }
       glColor3f(r, g, b);  
       glLogicOp(GL_COPY);
...
{    
    ...
    if(btn==GLUT_LEFT_BUTTON && state==GLUT_DOWN) 
    {
       glPushAttrib(GL_ALL_ATTRIB_BITS);
         opcao = ferramenta(x,y);  //determinar opção
	 if(opcao > 0) //caso seja selecionada alguma opção
         {
	        int i,d;  //inicializa variáveis
            rubber=0;
            count=0;
            draw_mode[0] = opcao;
	    switch(draw_mode[0])
	    {
        case(LINE): //definindo a variavel d e a opção escolhida
            d=0;                   
            draw_mode[1] = opcao;
	        break;
		case(RECTANGLE):
        ...
        case(NADA):           		   
            ...
            break;
        case(PRETO):                //definindo cores correntes
            r= 0; g= 0; b= 0; 
            draw_mode[0]=draw_mode[1];               
            break;
        case(BRANCO):    
       ...
       case(AMARELO):
            ...
            draw_mode[0]=draw_mode[1];
		    break;
       case(ISIZE): //atualizar tamanho da borracha
            size = 1.2 * size;
            draw_mode[0]=draw_mode[1];                   		    
		    break;
       case(DSIZE): //atualizar tamanho da borracha
           size = size/1.2;
           draw_mode[0]=draw_mode[1];                   	    
		   break;
       case(FILLOFF): //atualizar valor de fill e id           
		   id[6]=2;
           id[7]=1;
           fill = 0;
           draw_mode[0]=draw_mode[1];                   
           break;
       case(FILLON):  //atualizar valor de fill e id  
		   id[6]=1;
           id[7]=2;
           fill = 1;
           draw_mode[0]=draw_mode[1];                
		   break;
       case(CLEAR):
           clear(); //chamar função para limpar área de desenho        
           draw_mode[0]=draw_mode[1];		    
		   break;
     }
       if (opcao<7)
       ...
       switch(draw_mode[0])
	    {
		    ...
		    case (TRIANGLE):
	          ...
	          break;
		case(BORRACHA):
              glColor3f( rb, gb, bb);  //definir cor de fundo para a borracha
		      borralapis(x,y);                  
		     break;  
		case(TEXTO):
		      rx=x;
              ...
              break;
        case(LAPIS):
             glColor3f( r, g, b); //definir cor corrente para o lapis
		     borralapis(x,y);                   
		     break;                               
            }
         }
    ...
    else if(btn==GLUT_RIGHT_BUTTON && state==GLUT_DOWN) //para mudar a cor de fundo
    {
       glPushAttrib(GL_ALL_ATTRIB_BITS);
         int i;
	   ...
	   if(opcao > 0) 
         {  
            int m=0; 
            draw_mode[0] = opcao;
            switch(draw_mode[0]) //atualizar valores da cor de fundo
            {
                case(PRETO):
                   rb= 0; gb= 0; bb= 0; m=1;
                break;
            ...
            if (m==1) //se foi alterada a cor de fundo
            {
                for(i=0;i<6;i++)	        
                  id[i]=1;    
                  clear(); //limpar tela e deixar somente o fundo com a cor escolhida
            }      
         }    
       glPopAttrib();
    }
    else if (btn == GLUT_LEFT_BUTTON && state == GLUT_UP) //ao soltar o botao esquerdo
    {
       glPushAttrib(GL_ALL_ATTRIB_BITS);
          if (rubber==1) 
          {
             switch(draw_mode[0])
             {
	         case(LINE): //ao selecionar o ponto final da linha, desenhá-la        
		       xmm = x;
		       ymm = (wh - y);
		       glBegin(GL_LINES);
		        glVertex2f(xm, ym);
		       ...
		       glutSwapBuffers();
             break;
             case(RECTANGLE): //ao selecionar o ponto final do retângulo, desenhá-lo
                if(!fill) //retângulo não preenchido
                  glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);  	   
		          xmm = x;
		          ymm = (wh - y);
                  glRectf(xm, ym, xmm, ymm);
               if(fill) //retênagulo preenchido
                 { 
                  glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
                  glRectf(xm, ym, xmm, ymm);
                 }
                  ...
                  glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); 
                  break;
             case(TRIANGLE): //ao selecionar o ponto final do triângulo, desenhá-lo
                if(count==2)
                  {
                   xn = x;
	               yn = (wh - y);
	               if(fill) //se for preenchido
	                {
                     glBegin(GL_POLYGON);
                      glVertex2f(xm, ym);
			          glVertex2f(xn, yn);
                ...
                else //se não deve ser preenchido
	                {
	                 glBegin(GL_LINE_STRIP);
			          glVertex2f(xm, ym);
			          glVertex2f(xn, yn);
...
{
   if(draw_mode[0]!=TEXTO)   //caso a opção não seja a de escrever
   { 
        switch (k)
        { 
	    case 'q': 
            ...
            break;
        case 'c': 
		    clear();  //limpara área de desenho
            break;
	    case '+':
		    size = 1.2 * size; //aumentar tamanho da borracha
	        break;
	    case '-':
		    size = size/1.2;  //diminuir tamanho da borracha
	        break;	
        }
        glutPostRedisplay();
   }
   else  //se a opção escolhida for a de escrever
   {
	glColor3f(r,g,b); //cor corrente
	glRasterPos2i(rx,ry); //ir para a posição selecionada
	glutBitmapCharacter(GLUT_BITMAP_9_BY_15, k); //mostar letra digitada com a cor corrente
	rx+=glutBitmapWidth(GLUT_BITMAP_9_BY_15,k); //incrementar valor de rx para a letra aparecer ao lado da próxima
	glutSwapBuffers();
   }   
}
...
{
    if (id<=1)
      glColor3f(rf, gf, bf); //cor de fundo das opções (padrão)
    else if (id==2)
      glColor3f(re, ge, be); //cor de fundo da opção que for selecionada
    else if (id==3)
      ...
      glColor3f(0, 1, 1); //cor ciano
    else if (id==9)
      glColor3f(1, 0, 1); //cor magenta
    else if (id==10)
      glColor3f(1, 1, 0); //cor amarela
    glBegin(GL_QUADS);  
      glVertex2i(x,   y  );
      glVertex2i(x+s, y  );     //desenhando os quadrados de cores
      glVertex2i(x+s, y+s);
      glVertex2i(x,   y+s);
    glEnd(); 
    glColor3f(0.0, 0.0, 0.0); //cor das linhas dos quadrados que englobam as opções  
    glBegin(GL_LINE_LOOP);
      glVertex2i(x,   y  );
      glVertex2i(x+s, y  );     //formando os quadrados com um loop
      glVertex2i(x+s, y+s);
      glVertex2i(x,   y+s);
    glEnd();     
}

void display(void)  //desenhando o menu de opções
{
    int shift=0; //variavel que guarda o 
    glPushAttrib(GL_ALL_ATTRIB_BITS); 
             
       screen_box(0       , wh-ww/20, ww/20, id[0]);    //quadrados das opções do paint
       ...
       glBegin(GL_LINES);  //desenho da opção de linha
	   glVertex2i(wh/80,wh-ww/40);
	   glVertex2i(wh/80+ww/40,wh-ww/40);
       glEnd();
       
       glBegin(GL_POLYGON); //desenho da opção de quadrado
       ...
       glBegin(GL_TRIANGLES); //desenho da opção de triângulo
           glVertex2i(ww/10+ww/80,wh-ww/20+ww/80);
           glVertex2i(ww/10+ww/40,wh-ww/80);
           glVertex2i(ww/10+3*ww/80,wh-ww/20+ww/80);
       glEnd();
       
       glRectf(3*ww/20 + ww/60, wh-ww/20 + ww/80,3*ww/20 + ww/30,wh-ww/20+ ww/40); //parte inferior da borracha
       glBegin(GL_LINE_STRIP);
          glVertex2i(3*ww/20 + ww/30, wh-ww/20+ ww/40);
          glVertex2i(3*ww/20 + ww/30, wh-ww/20+ ww/30);   //parte superior da borracha
	      glVertex2i(3*ww/20 + ww/55, wh-ww/20+ ww/30);
          glVertex2i(3*ww/20 + ww/55, wh-ww/20+ ww/42);        
       glEnd();
       
       glRasterPos2i(ww/5 + 6,wh-ww/40 );
       glutBitmapCharacter  (GLUT_BITMAP_8_BY_13, 'A');
       shift=glutBitmapWidth(GLUT_BITMAP_8_BY_13, 'A');
       glRasterPos2i(ww/5+ 6+shift,wh-ww/40 -5);          //desenho da opção de texto
       glutBitmapCharacter   (GLUT_BITMAP_8_BY_13, 'B');
       shift+=glutBitmapWidth(GLUT_BITMAP_8_BY_13, 'B');
       glRasterPos2i(ww/5+ 6+shift,wh-ww/40 -10);
       glutBitmapCharacter(GLUT_BITMAP_8_BY_13, 'C');

       glBegin(GL_LINES);
          glVertex2i(ww/4 +ww/40-3  , wh-ww/20+ ww/70);
          glVertex2i(ww/4 +ww/40-3 , wh-ww/20+ ww/30);
          glVertex2i(ww/4 +ww/40+3  , wh-ww/20+ ww/70);   //parte inferior do lápis
          glVertex2i(ww/4 +ww/40+3 , wh-ww/20+ ww/30);
          glVertex2i(ww/4 +ww/40-3  , wh-ww/20+ ww/70);
	      glVertex2i(ww/4 +ww/40+3  , wh-ww/20+ ww/70);		           
       glEnd(); 
       glBegin(GL_TRIANGLES);
           glVertex2f(ww/4 +ww/40-4 , wh-ww/20+ ww/30);
           glVertex2f(ww/4 +ww/40+3  , wh-ww/20+ ww/30);  //ponta do lápis
           glVertex2f(ww/4 +ww/40+0.1,wh-ww/20+ ww/25);
       glEnd();      
        
       glColor3f(rb, gb, bb);
       glBegin(GL_POLYGON); //quadrado inferior contendo a cor de fundo
       ...
       glEnd();
       glColor3f(0.0, 0.0, 0.0);
       glBegin(GL_LINE_LOOP); //linha preta contornando o quadrado 
          glVertex2i(13*ww/20+ ww/40-5 , wh-ww/20 + 5);
          glVertex2i(13*ww/20+ ww/20 -5 , wh-ww/20 + 5);
          glVertex2i(13*ww/20+ ww/20 -5 , wh-ww/20 + 5 +ww/40);
          glVertex2i(13*ww/20+ ww/40-5, wh-ww/20 + 5 +ww/40);
       glEnd();
       glColor3f(r, g, b); //quadrado superior com a cor corrente
       glBegin(GL_POLYGON);
       ...
       glEnd();
       glBegin(GL_LINE_LOOP); //quadrado da opção fill off
       ...
       glEnd();
       glBegin(GL_POLYGON);  //quadrado da opção fill on
       ...
       glRasterPos2i(9*ww/10 +6 ,wh-ww/40 -5); //desenho da opção limpar 
       glutBitmapCharacter  (GLUT_BITMAP_8_BY_13, 'C');
       ...
       glBegin(GL_LINES); //desenho da opção de sair
...
int main(int argc, char** argv) //chamando funções
{
    glutInit(&argc,argv);
