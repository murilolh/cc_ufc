...
float max_elem_MD(float M[MAX][MAX], int d);//Obtem o maior elemento fora da diagonal de uma matriz
...
void jacobi(int d, float E, float M[MAX][MAX], float R[MAX][MAX]);//Acha os autovalores de M gerando a matriz R com os autovetores correspondentes
void result_inicio(int d, int t, float E, float M[MAX][MAX]);//Gera os resultados iniciais em um arquivo
void result_final(int d, float A[MAX][MAX], float X[MAX][MAX]);//Gera o resultado final em um arquivo
