/*
inverso.c
Funções de leitura e escrita no arquivo, de manipulacao de vetores e matrizes necessarias para o metodo da potencia inverso*/
#include "inverso.h" 
...
void ler_dados(FILE *fd, float dados[MAX])
{
    ...
    while(va[count]!=' ')  
        count++;           
    dados[k+1]=atoi(&va[count+1]);  //Armazena a toleracia.
}
...
//Resolve o sistema por fatoracao LU
void fat_LU(float M[MAX][MAX], float x[], int d, float y[])
{
    int i,j,k;
    float L[MAX][MAX];
    float U[MAX][MAX];
...
void result_inicio(int d, int t, float E, float M[MAX][MAX], float chute[], float lambda)
{
    int i,j;
    FILE *saida;
    saida  = fopen("output.txt","w");
    fprintf(saida,"\n********************** METODO DA POTENCIA INVERSO **********************\n\n");
