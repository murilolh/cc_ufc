...
//                                                             *
// Método da Potência Deslocado                                *
//                                                             *
//**************************************************************

#include "deslocado.h" //Arquivo com funções de leitura e escrita no arquivo, 
#include <stdio.h>   //de manipulacao de vetores e matrizes necessarias para o metodo da potencia deslocado
#include <stdlib.h>
#include <cmath>
...
{   
    ...
    float I[MAX][MAX];//Matriz identidade
    ...
    mu= dados[j+1];
    t=(int)dados[j+2];//Tolerancia em casas decimais
    ...
    //Inicializar matriz com os valore de mu na diagonal(mu*I)
    for(i=1;i<=d;i++)
       for(j=1;j<=d;j++)
       {
          if(i==j)
    ...
    sub_MM(M,I,M,d);//Acha M = M - mu*I
    igualar_vetores(y0,chute,d);
    lambda[0]=modulo(y0,d);
    result_inicio(d,t,mu,E,M,chute,lambda[0]);
    k=0;
    do
    {
