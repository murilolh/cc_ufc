...
gaussseidel::gaussseidel(double (*M)[MAX], double* T, int N, double E): metodo(M, T, N, E){};

int gaussseidel::inversa(){
	
	if (criteriodesassenfeld() == 1)
	{
		cout << "Criterio de Sassenfeld satisfeito!" <<endl;
		getchar();
		getchar();
	}
	else
	{
		cout << "Criterio de Sassenfeld nao satisfeito!" << endl;
		getchar();
		getchar();
	}
	...
	for (int c=0; c<N; c++)  // para cada coluna da matriz de coeficientes A
	{
		for (int j=0; j<N; j++)  
		{
			B[j] = I[j][c];   // atribui ao vetor B os elementos de cada coluna da matriz identidade
			X1[j] = X[j];    //atualiza os valores do vetor resultado a cada iteração
		}	
		while(true)
		{
			for (int i=0;i<N;i++)
				X1[i] = ((B[i] + somatorio(X1,i))/M[i][i]);  //calcula os valores da coluna usando o metodo
